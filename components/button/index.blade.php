...
@php
$loadingClass = $loading ? 'lux-button-loading' : '';
@endphp

<{{ $tag }} {{ $attributes->mergeTailwind(['class' => "lux-button lux-button-variant-{$variant} lux-button-size-{$size} $loadingClass"]) }}>
    {{ $slot }}
    @if($loading)
    <span class="lux-button-spinner">
